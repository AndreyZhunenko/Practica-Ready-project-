...
function New_continents(){ //отображение материков
	...
	let continent_properties = 'stroke:gray;stroke-width:0.1';
		...
		if (lines[i] != ""){
			...
			if (i == 0){
				...
				if (line_null != -1){
					...
					if (  (FinishPointContinent.x > -180 && FinishPointContinent.x < 180) 
						&& (FinishPointContinent.y > 30 && FinishPointContinent.y < 90)
						&& (StartPointContinent.x > -180 && StartPointContinent.x < 180)
						&& (StartPointContinent.y > 30 && StartPointContinent.y < 90) ) {

						let point1 = Gnomonic_projection_radar(StartPointContinent); //координаты радара
						let point2 = Gnomonic_projection_radar(FinishPointContinent); //физические координаты точки на луче

						New_Beams(StartPointContinent, FinishPointContinent, continent_properties);
					}
...
function Create_SVG(){ //рисуем холст для изображения
	...
	my_svg.id = "svg_id";
	my_svg.setAttribute('height', '700');
...
function New_Beams(Main_radar, My_beam, my_properties){ //отрисовка лучей
	

	var New_beam = document.createElementNS("http://www.w3.org/2000/svg","line");
	
	
	let point1 = Gnomonic_projection_radar(Main_radar); //координаты радара
	let point2 = Gnomonic_projection_radar(My_beam); //физические координаты точки на луче
	...
	New_beam.setAttribute('style', my_properties);
...
function Main_logic(){ //главная функция - полная отрисовка изображения
	...
	let beam_properties = 'stroke:black;stroke-width:0.2';
	...
	for (var i = 0; i < Beams.length; i++){
		...
		New_Beams(Main_radar, point, beam_properties);
	}
